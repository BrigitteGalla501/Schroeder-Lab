...
##plt.autoscale(enable=True, axis='both', tight=None) #sets the X and Y axises to auto scale
...
sampleLength = 5500 #This is how many loops the program will be through before stopping.

# Create a while loop that continuously measures and plots data from the keithley forever.

with open('Experiment1_Graphite.csv', 'w', newline='') as file:
    ...
    for i in range(sampleLength):
        ...
        ##plt.yticks(np.arange(len(voltageList)))
        ...
        if i == sampleLength -1:
            print("Saving figure!!!")
            plt.savefig('Experiment1_GraphiteGraph.png', bbox_inches = 'tight', pad_inches = .5) ## Names the file of the graph and saves it. Also it formats the file into a png and makes the image have nice margins. 
